...
interface ChallengeQuizBoxProps {
	userName: string;
	clues: string[];
	trivia: string[];
	qbid: string;
...
const ChallengeQuizBox: React.FC<ChallengeQuizBoxProps> = ({
	userName,
	clues,
	trivia,
	qbid,
	...
	const { userChallengeRecord, updateUserChallengeStats } = useChallengeStore();

	const { setWins, setLoss } = useChallengeStore();

	const router = useRouter();
	// guess button click handler
	const handleGuessClick = async () => {
		// Handle guess button click logic here
		const response = await validateAnswer(qbid, guess, token!, challengeCode);
		if (!response) {
			console.error("Error validating answer");
			return;
		}

		console.log("Response from validateAnswer challenge:", response);
	...
	const handleGiveUp = async () => {
		...
		const response = await getCorrectAnswer(qbid, token, challengeCode);
		if (!response) {
		...
		if (userChallengeRecord) {
			updateUserChallengeStats(userChallengeRecord.wins, response.data.loss);
		}
...
export default ChallengeQuizBox;
